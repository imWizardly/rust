        ...
        void cmdChatCleanInv(BasePlayer player, string command, string[] args)
        {
            if (IsAllowed(player, "inventorycleaner.allowed") && player != null)
            {
                if (args.Length == 0)
                {
                    ...
                    SendChatMessage(player, "<color=#41ff1c>Inventory Cleaner</color>: Your Complete Inventory is now clean!");
                    return;
                }
                if (args.Length == 1)
                {
                    switch (args[0])
                    {
                        case "help":
                            ...
                            sb.Append("<size=22><color=#41ff1c>Inventory Cleaner by TheDoc</color></size> v" + Version + " <color=#ce422b>http://www.uprisingserver.com</color>\n\n");
                            sb.Append("<color=#ff3d3d>Warning:</color> Once items removed they are GONE !").Append("\n\n");
                            sb.Append("<color=#41ff1c>Available commands</color> (ci or cleaninv) :").Append("\n");
						...
						case "belt":		
                            ...
                            SendChatMessage(player, "<color=#41ff1c>Inventory Cleaner</color>: Your Belt is now clean!");
                            break;
                        case "main":
                            foreach (var item in player.inventory.containerMain.itemList)
                            {
                            ...
                            SendChatMessage(player, "<color=#41ff1c>Inventory Cleaner</color>: Your Main Inventory is now clean!");
                            break;
                        case "both":
                            foreach (var item in player.inventory.containerBelt.itemList)
                            {
                            ...
                            SendChatMessage(player, "<color=#41ff1c>Inventory Cleaner</color>: Your Belt and Main Inventory is now clean!");
                            break;							
						case "fix":
