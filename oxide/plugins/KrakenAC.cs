...
{
    [Info("KrakenAC", "MrSleepy", "1.1.4")]
    [Description("1.1.4 Release (Stable)")]
    class KrakenAC : CovalencePlugin
    {
        [PluginReference] private Plugin Friends, PlaytimeTracker;

        private static KrakenAC _instance;
        ...
        private class Chat
        {
            ...
            public static void Send(BasePlayer player, Key key, Dictionary<string, string> parameters = null)
            {
                ...
                player.SendConsoleCommand("chat.add", 0, 76561199079761901UL, message);
            }
        }
        ...
        private class Command
        {
            ...
            public static void Load()
            {
                info = new List<Info>
                {
                    new Info(Config,   Key.CommandConfigTitle,   "kac.config"),
                    new Info(Help,     Key.CommandHelpTitle,     "kac.help"),
                    new Info(Ip,       Key.CommandIpTitle,       "kac.ip"),
                    new Info(Log,      Key.CommandLogTitle,      "kac.log"),
                    new Info(Server,   Key.CommandServerTitle,   "kac.server"),
                    new Info(Teleport, Key.CommandTeleportTitle, "kac.tp", "kac.tpv"),
                    new Info(Users,    Key.CommandUserTitle,     "kac.user"),
                    new Info(Vpn,      Key.CommandVpnTitle,      "kac.vpn")
                };
        ...
        private class Configuration
        {
            ...
            public static void Load()
            {
                ...
                {
                    KrakenAC.Log.Warning("Configuration: Created new configuration with default settings.");

                    dirty = true; config = new Configuration();
                }
                catch (JsonException e)
                {
                    KrakenAC.Log.Error("Configuration: Using default settings. Delete the configuration file, or fix the following error, and reload; " + e.ToString());
            ...
            public class Log
            {
                ...
                public static void Add(Weapon weapon)
                {
                    ...
                    foreach (var projectileid in weapon.Projectiles)
                    {
                        ...
                        {
                            KrakenAC.Log.Warning($"Projectile.Log.Add({projectileid}): {weapon.Name}({e.Message})");
                        }
                    }
                }
        ...
        private class User
        {
            ...
            public static List<ulong> Team(ulong userid)
            {
                ...
                if (teamid != 0)
                {
                    return new List<ulong>(RelationshipManager.Instance.teams[teamid].members);
                }
        ...
        private class Violation
        {
            ...
            private void Broadcast(ulong userid, Key action, Key type, string details, Dictionary<string, string> hook_details)
            {
                ...
                switch (category)
                {
                    case Key.AntiCheat: parameters["color"] = 0xff6060.ToString(); parameters["categoryicon"] = ":kraken:"; break;
        ...
        private class VPN
        {
            public class Settings
            {
                ...
                public class VpnApi
                {
                    public KrakenAC.API.Settings GetIpIntel;
                    public KrakenAC.API.Settings IpApi;
                    public KrakenAC.API.Settings IpHub;
                    public KrakenAC.API.Settings IpQualityScore;

                    public VpnApi()
                    {
                        GetIpIntel = new KrakenAC.API.Settings();
                        IpApi = new KrakenAC.API.Settings();
                        IpHub = new KrakenAC.API.Settings();
                        IpQualityScore = new KrakenAC.API.Settings();
                    }

                    public void Validate()
                    {
                        Configuration.Validate(ref GetIpIntel, () => new KrakenAC.API.Settings(), () => GetIpIntel.Validate());
                        Configuration.Validate(ref IpApi, () => new KrakenAC.API.Settings(), () => IpApi.Validate());
                        Configuration.Validate(ref IpHub, () => new KrakenAC.API.Settings(), () => IpHub.Validate());
                        Configuration.Validate(ref IpQualityScore, () => new KrakenAC.API.Settings(), () => IpQualityScore.Validate());
                    }
                }
